...
from simulado_e_cards.form import (
    FormCard,
    FormMateria,
    FormQuestao,
    FormAnotacao,
...
def flashcard():
    ...
    if "resultado" in request.form:
        ...
        resposta = RespostaCard(resultado=resultado, card_id=card_id)
    ...
    return render_template(
        "flashcards.html",
        form_card_conectado=form_card_conectado,
        cards=cards,
        materias=materias,
    )
...
def materias():
    ...
    if "botao_submit_editar" in request.form:
        ...
        if materia:
            ...
            form_materia.cor.data = materia.cor
    ...
    if "botao_salvar_edicao" in request.form:
        ...
        if materia:
            ...
            materia.cor = form_materia.cor.data
...
def materia(materia_id):
    ...
    if "botao_submit_anotacao" in request.form:
        if form_anotacao.validate_on_submit():
            anotacao = Anotacao(
                ...
                tags=form_anotacao.tags.data,
                materia_id=materia_id,
            )
            database.session.add(anotacao)
            database.session.commit()
            return redirect(url_for("materia", materia_id=materia_id) + "#anotacoes")
    if "botao_submit_excluir_anotacao" in request.form:
        anotacao_id = request.form.get("anotacao_id")

        anotacao = Anotacao.query.filter_by(
            id=anotacao_id, materia_id=materia_id
        ).first()

        if anotacao:
            database.session.delete(anotacao)
            database.session.commit()

        return redirect(url_for("materia", materia_id=materia_id) + "#anotacoes")
    if "botao_submit_editar_anotacao" in request.form:
        anotacao_id = request.form.get("anotacao_editar_id")
        ...
        if anotacao:
            anotacao_edicao = anotacao

            form_anotacao.titulo.data = anotacao.titulo
            form_anotacao.conteudo.data = anotacao.conteudo
            form_anotacao.tags.data = anotacao.tags

            modal_aberto = "editar_anotacao"

    if "botao_salvar_edicao_anotacao" in request.form:
        anotacao_id = request.form.get("anotacao_editar_id")
        ...
        if anotacao:
            anotacao.titulo = form_anotacao.titulo.data
            anotacao.conteudo = form_anotacao.conteudo.data
            anotacao.tags = form_anotacao.tags.data
    ...
    return render_template(
        ...
        cards=cards,
        modal_aberto=modal_aberto,
        card_edicao=card_edicao,
        anotacoes=anotacoes,
        anotacao_edicao=anotacao_edicao
    )


@app.route("/anotacao", methods=["GET", "POST"])
def anotacao():
    form_anotacao_conectado = FormAnotacaoconectado()

    materias = Materia.query.all()

    form_anotacao_conectado.materia_id.choices = [
        (materia.id, materia.nome_materia) for materia in materias
    ]

    if form_anotacao_conectado.validate_on_submit():
        anotacao = Anotacao(
            titulo=form_anotacao_conectado.titulo.data,
            conteudo=form_anotacao_conectado.conteudo.data,
            tags=form_anotacao_conectado.tags.data,
            materia_id=form_anotacao_conectado.materia_id.data,
        )
        database.session.add(anotacao)
        database.session.commit()
        return redirect(url_for("anotacao") + "#anotacoes")
